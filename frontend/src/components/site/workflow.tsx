...
export function Workflow() {
  return (
    <section className="px-4 py-16 sm:px-6 lg:px-8" id="docs">
      <div className="mx-auto w-full max-w-7xl rounded-3xl border border-white/10 bg-[#050505]/88 p-8">
        <h2 className="text-3xl font-semibold text-white">Workflow in Seconds</h2>
        <div className="mt-8 grid gap-4 lg:grid-cols-4">
          {steps.map((step, index) => (
            <motion.div
              key={step}
              className="relative rounded-xl border border-white/10 bg-[#0f0f0f]/80 p-4"
            ...
            >
              ...
              {index < steps.length - 1 ? (
                <motion.span
                  className="absolute top-1/2 -right-3 hidden h-0.5 w-6 bg-gradient-to-r from-white/40 to-transparent lg:block"
